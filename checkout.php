...
if(isset($_POST['order'])){
    ...
    $cart_total = mysqli_real_escape_string($conn, $_POST['totalprice']);

    if($cart_total == 0){
        $message[] = 'your cart is empty!';
    }else{
        mysqli_query($conn, "INSERT INTO `orders`(user_id, number, method, address, total_price, placed_on) VALUES('$user_id', '$number', '$method', '$address', '$cart_total', '$placed_on')") or die('query failed');
        ...
        if(mysqli_num_rows($select_cart) > 0){
            ...
            $total_price = ($fetch_product['price'] * $qty);
            mysqli_query($conn, "INSERT INTO `orderitem`(oid, itemid, qty, tprice) VALUES ($order_id, $productid, $qty, $total_price)")  or die('query failed');
            }
        }
...
<section class="display-order">
    <?php
        ...
        if(mysqli_num_rows($select_cart) > 0){
            ...
            $select_product = mysqli_query($conn, "SELECT * FROM `products` WHERE id = '$productid'") or die('query failed');
            $fetch_product = mysqli_fetch_assoc($select_product);
            $total_price = ($fetch_product['price'] * $fetch_cart['quantity']);
            $grand_total += $total_price;
    ?>    
    <p> <?php echo $fetch_product['name'] ?> <span>(<?php echo '₹'.$fetch_product['price'].'/-'.' x '.$fetch_cart['quantity']  ?>)</span> </p>
    <?php
        }
        }else{
...
<section class="checkout">
    ...
    <form action="" method="POST">
        <input type="hidden" value="<?php echo $grand_total; ?>" name="totalprice">
